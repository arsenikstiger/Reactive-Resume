...
import aurelienStyles from "./Aurelien.css";
...
const Aurelien = () => {
    ...
    <header style={styles.header}>
      <div className="ml-6">
        <h1 className="tracking-wide uppercase font-semibold" style={{ fontSize: '2.75em' }}>
          {data.profile.firstName} {data.profile.lastName} <span aria-label="emoji-check" role="img" className="emoji">✔️</span>
        </h1>
  ...
  const AddressItem = ({ label, line1, line2, line3 }) =>
  (line1 || line2 || line3) && (
  <div className="mb-3">
    <h6 className="text-xs font-bold">{label}</h6>
  ...
  const Contact = () => (
    <div className="mb-6">
      ...
      <AddressItem label={data.contact.address.heading} line1={data.contact.address.line1} line2={data.contact.address.line2} line3={data.contact.address.line3} />
      <ContactItem label={data.contact.email.heading} value={data.contact.email.value} />
      <ContactItem label={data.contact.phone.heading} value={data.contact.phone.value} />
      <ContactItem label={data.contact.website.heading} value={data.contact.website.value} />
    </div>
  );

  const WorkItem = x => (
    <div key={x.id} className="workitem my-3 mr-10 pt-2">
      <div>
        <h6 className="font-semibold">{x.title} | {x.start} - {x.end}</h6>
      </div>
  ...
  const EducationItem = x => (
    <div key={x.id} className="my-2">
      ...
      <p className="text-xs">{x.name} | {x.start} - {x.end}</p>
      <ReactMarkdown className="mt-2 text-sm" source={x.description} />
    </div>
  );
  ...
  const Education = () =>
    ...
    data.education.enable && (
      <div className="mb-6">
        <Heading title={data.education.heading}  className="w-3/4 mx-auto" />
  ...
  const Skills = () =>
    data.skills.enable && (
      <div className="mb-6">
        <Heading title={data.skills.heading} className="w-3/4 mx-auto" />
  ...
  const Hobbies = () =>
    data.hobbies.enable && (
      <div className="mb-6">
        <Heading title={data.hobbies.heading} className="w-3/4 mx-auto" />
        <ul className="list-none text-sm">
    ...
    >
      <div className="grid grid-cols-12">
        ...
        <div className="col-span-8">
          ...
          <section className="py-4 pb-0" style={styles.section}>
            <Objective />
            <Work />
            <Awards />
            <References />
